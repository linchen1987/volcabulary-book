...
export default function WordListPage() {
  ...
  const typeFilter = (searchParams.get('type') || 'all') as 'all' | 'word' | 'phrase' | 'base-word';
  ...
  const loadWords = useCallback(
    ...
    [spaceId, q, levelFilter, typeFilter, sortBy, sortOrder],
  );
  ...
  const setTypeFilter = (value: 'all' | 'word' | 'phrase' | 'base-word') => {
    const params = new URLSearchParams(searchParams);
    if (value === 'all') {
      params.delete('type');
    } else {
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <select
              value={typeFilter}
              onChange={(e) =>
                setTypeFilter(e.target.value as 'all' | 'word' | 'phrase' | 'base-word')
              }
              className="h-9 rounded-lg border border-input bg-background px-3 py-1.5 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 cursor-pointer"
            >
              ...
              <option value="base-word">词族</option>
            </select>
          </div>
          ...
          {words?.length === 0 && !isLoading && (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <h3 className="text-lg font-semibold">
                {q || levelFilter !== undefined || typeFilter !== 'all'
                  ? '没有找到匹配的单词'
                  : '暂无单词'}
              </h3>
